...
function fuseResults(
  ...
  string,
  { score: number; keywordRank?: number; semanticScore?: number }
> {
  ...
  for (const result of keywordResults) {
    const normalizedRank = normalizeScore(
      result.rank,
      minKeywordRank,
      maxKeywordRank,
    );
...
const app = createApp()
  ...
  .get("/search/keyword", async (c) => {
    try {
      ...
      const noteType = searchParams.get("noteType") as
        | "note"
        | "journal"
        | null;
      const minMoodScore = searchParams.get("minMood")
        ? // biome-ignore lint/style/noNonNullAssertion: exists, checked in the ternary
      ...
      if (
        minMoodScore !== undefined &&
        (minMoodScore < 1 || minMoodScore > 10)
      ) {
        return c.json(
      ...
      if (
        maxMoodScore !== undefined &&
        (maxMoodScore < 1 || maxMoodScore > 10)
      ) {
        return c.json(
      ...
      return c.json({
        ...
        pagination: {
          ...
          hasMore: includeCount
            ? // biome-ignore lint/style/noNonNullAssertion: if includeCount is true, totalCount is not undefined
              offset + results.length < totalCount!
            : undefined,
        },
      });
    } catch (error) {
