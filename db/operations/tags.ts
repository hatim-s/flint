...
import { type Tag, tags } from "@/db/schema/tags";
 ...
 */
async function getNoteTags(userId: string, noteId: string): Promise<Tag[]> {
  const rls = rlsExecutor(userId);
  ...
  const result = await db
    ...
    .where(and(rls.where(tags), rls.where(notes, eq(notes.id, noteId))));
